<script>
	import { createProvince } from '$providers/actions/ktour/province';
	...
	const onCreateProvince = async (evt) => {
		evt.preventDefault();
		let place = await createProvince.load({
			name
		});
		if (place?.success) disabledCreate = true;
		dispatch('onCreate');
	};
</script>
		...
		<div class="relative w-80 h-[200px]">
			<!-- <div class="absolute inset-0 py-6 px-6 lg:px-8"> -->
				<form
					...
					on:submit|preventDefault={onCreateProvince}
					method="post"
				>
					<div class="flex-grow">
						<div>
