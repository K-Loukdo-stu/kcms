<script>
	import { updateService } from '$providers/actions/kauth/service';
	...
	const updatedService = async (evt) => {
		evt.preventDefault();
		let res = await updateService.load({
			id,
			name,
			url,
			role
		...
		<div class="w-80 h-[400px]">
			...
			<div class="py-6 px-6 lg:px-8">
				<form
					...
					on:submit|preventDefault={updatedService}
					method="post"
				>
					<div class="flex-grow-0">
						<h3 class="text-xl font-medium text-gray-900 text-white text-center">Edit Service</h3>
					</div>
					<div class="flex-grow">
						<div class="mb-2">
							<label
