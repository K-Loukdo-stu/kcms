<script>
  ...
  import { updateKLoukdoAdsImage } from '$providers/actions/kloukdo/kloukdoadsimage';
  ...
  import { updateKLoukdoSubCategory } from '$providers/actions/kloukdo/kloukdosubcategory';
	...
	const updateAds = async (evt) => {
		evt.preventDefault();
		let res = await updateKLoukdoAdsImage.load({
			id: param.id, name, endDate
		});
		...
		<div class="w-80 h-[28-rem]">
			...
			<div class="py-6 px-6 lg:px-8">
				<form
					...
					on:submit|preventDefault={updateAds}
					method="post"
				>
