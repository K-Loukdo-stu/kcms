<script>
  ...
  import { updateKLoukdoPromotion } from '$providers/actions/kloukdo/kloukdopromotion';
  import { updateKLoukdoSubCategory } from '$providers/actions/kloukdo/kloukdosubcategory';
	...
	const updateSubCategory = async (evt) => {
		...
		let res = await updateKLoukdoPromotion.load({
			id: param.id, endDate:formattedEndDate
		});
