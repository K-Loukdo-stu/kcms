<script>
  import { createKLoukdoCategory } from '$providers/actions/kloukdo/kloukdocategory';
	...
	const createdService = async (evt) => {
		evt.preventDefault();
		let res = await createKLoukdoCategory.load({
			name, icon
		});
					...
					<div class="flex-grow">
						<div class="mb-2">
							...
							<input
								...
								name="name"
								bind:value={name}
								class="border text-center border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 bg-gray-600 border-gray-500 placeholder-gray-400 text-white"
								placeholder="Name"
								required
							/>
						</div>
