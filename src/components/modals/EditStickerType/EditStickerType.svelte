<script>
	import { updateStickerType } from '$providers/actions/kart';
	...
	const updateType = async (evt) => {
		evt.preventDefault();
		let res = await updateStickerType.load({
			id,
			name,
			url
		});
		...
		<div class="w-80 h-[310px]">
			...
			<div class="py-6 px-6 lg:px-8">
				<form
					...
					on:submit|preventDefault={updateType}
					method="post"
				>
					<div class="flex-grow-0">
						<h3 class="text-xl font-medium text-gray-900 text-white text-center">Edit Type</h3>
					</div>
					<div class="flex-grow">
						<div class="mb-2">
							<label
						...
						<div>
							<label
								for="url"
								class="block mb-2 text-sm font-medium text-gray-900 text-gray-300">Emoji</label
							>
							<input
								type="text"
								name="url"
