<script>
	...
	import { updateAlbum } from "$providers/actions/kart/music/album";
	...
	const onEditAlbumn = async (file) => {
		try {
			editAlbumState.loading = true;
			await updateAlbum.load({
				id: rawData.id,
				name,
				desc,
				releasedDate,
			...
			editAlbumState.errMsg = error?.message || "Unknown error";
		} finally {
			base64 = "";
			editAlbumState.loading = false;
		}
	};
	...
	const onUpload = async (evt) => {
		...
		try {
			if (base64) {
				...
				await onEditAlbumn(res.response);
			}
		} finally {
			base64 = "";
			editAlbumState.loading = false;
