<script>
	...
	const onCreateGenre = async (evt) => {
		evt.preventDefault();
		try {
			createGenreState.loading = true;
			await createGenre.load({
				name,
			});
			dispatch("create");
		} catch (error) {
			createGenreState.errMsg = error?.message || "Unknown error";
		} finally {
			createGenreState.loading = false;
		}
	};
</script>
		...
		<div class="w-80 max-h-[300px]">
			<div class=" py-6 px-6 lg:px-8 w-full h-full">
				...
				>
					...
					{#if createGenreState?.errMsg}
						<div
							class=" text-sm text-center bg-yellow-50 text-orange-700 rounded-md p-2"
						>
							{createGenreState.errMsg}
						</div>
					{/if}
					<div class="flex-grow-0">
						{#if createGenreState.loading}
