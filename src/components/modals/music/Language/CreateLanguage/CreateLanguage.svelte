<script>
	...
	import { createGenre } from "$providers/actions/kart/music/genre";
	import { createLanguage } from "$providers/actions/kart/music/language";
	...
	let createLanguageState = {
		loading: false,
		errMsg: "",
	};

	const onCreateLanguage = async (evt) => {
		evt.preventDefault();
		try {
			createLanguageState.loading = true;
			await createLanguage.load({
				name,
			});
			dispatch("create");
		} catch (error) {
			createLanguageState.errMsg = error?.message || "Unknown error";
		} finally {
			createLanguageState.loading = false;
		}
	};
</script>
				...
				>
					<div class="flex-grow">
						<div>
							...
							<h3
								class="mb-5 text-base font-medium text-gray-700 text-center"
							>
								New Language
							</h3>
							<label
