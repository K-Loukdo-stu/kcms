<script>
	...
	import { updateLanguage } from "$providers/actions/kart/music/language";
	...
	let editLanguageState = {
		loading: false,
		errMsg: "",
	};

	const onEditLanguage = async (evt) => {
		evt.preventDefault();
		try {
			editLanguageState.loading = true;
			await updateLanguage.load({
				id: rawData.id,
				name,
			});
			dispatch("edit");
		} catch (error) {
			editLanguageState.errMsg = error?.message || "Unknown error";
		} finally {
			editLanguageState.loading = false;
		}
	};
</script>
		...
		<div class="w-80 max-h-[300px]">
			<div class=" py-6 px-6 lg:px-8 w-full h-full">
				<form
					...
					on:submit|preventDefault={onEditLanguage}
					method="post"
				>
					<div class="flex-grow">
						<div>
					...
					{#if editLanguageState?.errMsg}
						<div
							class=" text-sm text-center bg-yellow-50 text-orange-700 rounded-md p-2"
						>
							{editLanguageState.errMsg}
						</div>
					{/if}
					<div class="flex-grow-0">
						{#if editLanguageState.loading}
