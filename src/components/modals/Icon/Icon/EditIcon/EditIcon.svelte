...
<script>
	...
	import { createIcon, updateIcon } from "$providers/actions/kart/icon";
	import { createIconCategory } from "$providers/actions/kart/icon-category";
	...
	let onModal = {
		shownModal: false,
		showType: false,
		selectedCategory: categoryData.find(
			(cat) => cat.id == icon.category.id
		),
	};
	...
	<div class=" absolute bg-white rounded-md w-80 h-3/4">
		...
		>
			...
			<SelectStickerType
				...
				on:select={(evt) => {
					onModal.selectedCategory = evt.detail.type;
