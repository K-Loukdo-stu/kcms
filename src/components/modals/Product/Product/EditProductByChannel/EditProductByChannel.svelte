<script>
	...
	import { updateProduct } from '$providers/actions/kchannel/product';
	...
	const onUpdate = async (evt) => {
		...
		if (base64) {
			...
			let res = await updateProduct.load({
				id:product.id,
				name,
				photo,
				price,
	...
	onMount(() => {
		productTypeSelected = productTypesData.find((cat) => cat.id == type.id);
	});
</script>
