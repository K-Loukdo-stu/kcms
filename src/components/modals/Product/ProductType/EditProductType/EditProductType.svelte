<script>
	...
	import { createProductType, updateProductType } from '$providers/actions/kchannel/product-type';
	import { uploadIconCategory } from '$providers/actions/storage/icon-category';
	import { createEventDispatcher, onMount } from 'svelte';

	const dispatch = createEventDispatcher();
	export let productTypeCatalogsData = [];
	export let rawData;

	let name = rawData.name;
	let icon = rawData.icon;
	let catalog = rawData.catalog;
	let disabledEdit = false;
	let base64;
	let uploadPercentage = 0;
	let onLoading = false;

	let productTypeCatalogsSelected = null;

	onMount(() => {
		productTypeCatalogsSelected = productTypeCatalogsData.find((cat) => cat.id == catalog.id);
	});
				...
				onComplete: (res) => {
					...
					updateProductType
						.load({
							...
							name,
							icon: res.response,
							catalog: productTypeCatalogsSelected.id
						})
						.then((res) => {
							// to do
							dispatch('edit');
					...
					<div class="flex-grow">
						<div>
							...
							<select
								...
								bind:value={productTypeCatalogsSelected}
							>
								{#each productTypeCatalogsData as data}
									<option value={data}>
										{data.name}
									</option>
								{/each}
