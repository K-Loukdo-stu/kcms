<script>
	...
	import { createProductOption } from '$providers/actions/kchannel/option';
	import { createProduct } from '$providers/actions/kchannel/product';
	import { createProductSubOption, updateProductSubOption } from '$providers/actions/kchannel/sub-option';
	import { uploadIcon } from '$providers/actions/storage/icon';
	import { createEventDispatcher } from 'svelte';

	const dispatch = createEventDispatcher();
	export let option;
	export let subOption;

	let disabledCreate = false;

	let name = subOption.name;
	let price = subOption.price;
	...
	const onUpdate = async (evt) => {
		...
		disabledCreate = true;
		onLoading = true;
		let res = await updateProductSubOption.load({
			id: subOption.id,
			name,
			price,
			option: option.id
