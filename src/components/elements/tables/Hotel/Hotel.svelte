...
<table class="table-auto w-full mt-5">
	<thead class="text-sm bg-[#787878] text-[#FFFFFF]">
		<tr>
			<th>
				<div class="font-semibold text-center">Hotel</div>
			</th>
			<th class="p-2">
		...
		{#each Data as data}
			<tr>
				{#if data.mediaUrl}
					<td class="p-2">
						<div>
							<a href={`/dashboard/ktour/hotel/hotel/detail/info/${data.id}`} title={data.name}>
								<img class=" w-10 h-10 m-auto" src={data.mediaUrl.url} alt="" />
							</a>
						</div>
					</td>
				{:else}
				...
				{#if data.province}
					<td class="p-2">
						<div class=" text-gray-600 text-center">{data.province.name}</div>
					</td>
				{:else}
					<td class="p-2">
				...
				<td class="p-2 space-x-2">
					<div class=" text-gray-600 text-center flex flex-row justify-center items-center">
						<a href={`/dashboard/ktour/hotel/hotel/detail/info/${data.id}`} title={data.name}>
							<div class="cursor-pointer hover:bg-gray-200 p-1 rounded-full">
								<Eye />
							</div>
						</a>
					</div>
