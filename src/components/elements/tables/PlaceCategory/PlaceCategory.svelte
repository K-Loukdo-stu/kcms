...
<table class="table-auto w-full mt-5">
	...
	<tbody class="text-sm divide-y divide-gray-300 text-[#585861]">
		{#each placeCategoryData as packages}
			<tr>
				{#if packages.mediaFile}
					<td class="p-2">
						<div>
							<img class=" w-10 h-10 m-auto" src={packages.mediaFile.url} alt="" />
						</div>
					</td>
				{:else}
					<td class="p-2">
				...
				{#if packages.placesCount}
					<td class="p-2">
						<div class=" font-medium text-gray-600 text-center">{packages.placesCount}</div>
					</td>
				{:else}
					<td class="p-2">
